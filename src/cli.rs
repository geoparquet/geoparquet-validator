//! The command line. `main.rs` is a shim over [`run`], so the Python package can offer the same
//! command without a second binary.

use crate::{checks, corpus, source, verify};
...
/// Run the command line with `args` (the first is the program name) and return the exit code:
/// 0 conformant, 1 a test failed, 2 the tool could not run.
